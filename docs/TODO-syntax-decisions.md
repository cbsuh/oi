...
### 기본 타입과 리터럴
- [ ] 숫자 타입: `Int`의 크기, `Int32`/`Int64`/`UInt` 여부
- [ ] 리터럴 문법: `0x` (hex), `0b` (binary), `_` 구분자 (`1_000_000`), 과학적 표기법
- [ ] 문자 타입: `Char` 존재 여부, 단일 문자 리터럴 (`'a'`)
- [ ] 컬렉션 리터럴: `Map`, `Set`, `Tuple` 리터럴 문법
- [ ] Unit / Void: `()` vs `Unit` 키워드
